                ...
                <div>
                    @if(count($musics) > 0)
                        @foreach($musics as $key=>$music)
                            <div
                                class="d-flex align-items-center @if($key %2 !== 0) success p-2 text-dark bg-opacity-10 @endif">
                                <a class="lesten" href="{{ route('client.music.detail', $music->id) }}">
                                    <img class="border shadow-sm p-3 m-2 bg-body rounded" src="{{ $music->image }}"
                                         width="160px" height="120px"/>
                                </a>
                                <div class="">
                                    <div class="border-bottom pb-3 ">
                                        <a class="lesten" href="{{ route('client.music.detail', $music->id) }}"><p
